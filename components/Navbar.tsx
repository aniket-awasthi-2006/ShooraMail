      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 backdrop-blur-md border-b transition-all duration-700 ease-in-out ${
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-8 h-20 flex items-center justify-between">
          ...
          >
            ...
            <span className={`font-black text-lg md:text-xl tracking-tighter transition-colors duration-700 ${
              isDark ? 'text-white' : 
          ...
          <div className="flex items-center gap-4 md:gap-8">
            <div className="hidden sm:flex items-center gap-6 text-[10px] md:text-sm font-black uppercase tracking-widest">
              ...
              <button 
                ...
                className={`transition-all duration-500 hover:opacity-100 ${currentView === 'signin' ? 'opacity-100' : 'opacity-40'} ${!isDark ? 'text-black' : 'text-white'}`}
              >
                Log In
              </button>
            </div>
        ...
        {showAuthModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
            ...
            <MotionDiv
              ...
              className={`relative w-full max-w-sm p-8 rounded-[32px] border shadow-2xl transition-colors duration-500 ${
                isDark ? 'bg-[#131416] border-[#25282B] text-white' : 'bg-white border-gray-100 text-black'
              }`}
            >
              <button 
                onClick={() => setShowAuthModal(false)}
                ...
                <form onSubmit={handleAdminAuth} className="w-full flex flex-col gap-4">
                  <div className="relative">
                    <input 
                      ...
                      className={`w-full px-5 py-4 rounded-2xl border-2 text-sm font-bold outline-none transition-all duration-300 ${
                        error 
                  ...
                  <button 
                    type="submit"
                    className={`w-full py-4 rounded-full font-black text-xs uppercase tracking-widest transition-all duration-500 shadow-xl ${
                      isColored ? 'bg-[#2D62ED] text-white' : 
